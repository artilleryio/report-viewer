...
const _mapToLegacyBaseLevelObject = (src) => {
  return {
    ...
    requestsCompleted:
      ...
      src.counters["http.responses"] || // Artillery 2.x.x
      0,
    ...
    rps: {
      mean: src.rates
        ? src.rates["engine.http.response_rate"] ||
          ...
          src.rates["http.response_rate"] || // Artillery 2.x.x
          0
        : 0,
      count:
        src.counters["engine.http.responses"] ||
...
const _mapToLegacyLatency = (src) => {
  ...
  if (src["http.response_time"]) { // Artillery 2.x.x
